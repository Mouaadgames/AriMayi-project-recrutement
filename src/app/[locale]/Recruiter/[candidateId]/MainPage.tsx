    ...
    <div >
      ...
      <div className="mx-auto border max-w-xl flex flex-col shadow-md rounded-md p-2 h-[80vh]">
        <div className="pl-4 flex flex-col gap-1 my-4">
          <p> <b>name</b> : {selectedCandidate.current?.name} <CopyButton text={selectedCandidate.current?.name} /></p>
          <p> <b>email</b> : {selectedCandidate.current?.email} <CopyButton text={selectedCandidate.current?.email} /></p>
          <p> <b>phone</b> : {selectedCandidate.current?.phone} <CopyButton text={selectedCandidate.current?.phone} /></p>
          <p> <b>address</b> : {selectedCandidate.current?.address} <CopyButton text={selectedCandidate.current?.address} /></p>
        </div>
          <p className="flex items-center justify-center gap-1 mb-2">Uploaded resume : <ViewPDF link={selectedCandidate.current?.cvLink}/></p>
        {/* @ts-ignore */}
